...
REQ_BODY.put("pmtprdNm",    StringUtil.isNull(request.getParameter("pmtprdNm")));       //상품명
REQ_BODY.put("mchtCustNm",  StringUtil.isNull(request.getParameter("mchtCustNm")));     //주문자명
REQ_BODY.put("mchtCustId",  StringUtil.isNull(request.getParameter("mchtCustId")));     //상점고객아이디
REQ_BODY.put("email",       StringUtil.isNull(request.getParameter("email")));          //이메일
REQ_BODY.put("billKey",     StringUtil.isNull(request.getParameter("billKey")));        //빌키
...
REQ_BODY.put("instmtMon",   StringUtil.isNull(request.getParameter("instmtMon")));      //할부개월수
REQ_BODY.put("crcCd",       StringUtil.isNull(request.getParameter("crcCd")));          //통화구분
REQ_BODY.put("taxTypeCd",   StringUtil.isNull(request.getParameter("taxTypeCd")));      //세금유형
...
REQ_BODY.put("taxFreeAmt",  StringUtil.isNull(request.getParameter("taxFreeAmt")));     //비과세금액
REQ_BODY.put("svcAmt",      StringUtil.isNull(request.getParameter("svcAmt")));         //봉사료
REQ_BODY.put("notiUrl",     StringUtil.isNull(request.getParameter("notiUrl")));        //결과처리URL
REQ_BODY.put("mchtParam",   StringUtil.isNull(request.getParameter("mchtParam")));      //기타주문정보
...
RES_BODY.put("vldDtMon", "");       //유효기간(월)
RES_BODY.put("vldDtYear", "");      //유효기간(년)
RES_BODY.put("issrId", "");         //발급사아이디
RES_BODY.put("cardNm", "");         //카드사명
RES_BODY.put("cardKind", "");       //카드종류명
RES_BODY.put("ninstmtTypeCd", "");  //무이자할부타입
RES_BODY.put("instmtMon", "");      //할부개월수
RES_BODY.put("apprNo", "");         //승인번호

//AES256 암호화 필요 파라미터
String[] ENCRYPT_PARAMS = {"cardNo","cardPwd", "idntNo", "vldDtMon","vldDtYear","trdAmt", "taxAmt", "vatAmt", "taxFreeAmt", "svcAmt"};
...
//URL설정
String requestUrl = serverURL + "/spay/APICardActionPay.do";
...
<div id="STPG_RSLT"> 
    <table>
        ...
        <tr>
            <td>trdAmt[거래금액]</td>
            <td><%= respParam.get("trdAmt") %></td>
        </tr>

        <tr>
            <td>cardNo[카드번호]</td>
            <td><%= respParam.get("cardNo") %></td>
        </tr>
        <tr>
            <td>vldDtYear[유효기간(년)]</td>
            <td><%= respParam.get("vldDtYear") %></td>
        </tr>
        <tr>
            <td>vldDtMon[유효기간(월)]</td>
            <td><%= respParam.get("vldDtMon") %></td>
        </tr>
        <tr>
            <td>issrId[발급사아이디]</td>
            <td><%= respParam.get("issrId") %></td>
        </tr>
        <tr>
            <td>cardNm[카드사명]</td>
            <td><%= respParam.get("cardNm") %></td>
        </tr>
        <tr>
            <td>cardKind[카드종류명]</td>
            <td><%= respParam.get("cardKind") %></td>
        </tr>
        <tr>
            <td>ninstmtTypeCd[무이자할부타입]</td>
            <td><%= respParam.get("ninstmtTypeCd") %></td>
        </tr>
        <tr>
            <td>instmtMon[할부개월수]</td>
            <td><%= respParam.get("instmtMon") %></td>
        </tr>
        <tr>
            <td>apprNo[승인번호]</td>
            <td><%= respParam.get("apprNo") %></td>
        </tr>
        <tr style="background-color:yellow;">
            <td>billKey[빌키]</td>
            <td><%= respParam.get("billKey") %></td>
        </tr>

        <tr>
            <td colspan="2" style="text-align: center;">
            	<input type="button" value="돌아가기" style="margin-top:20px;" onclick="location.href='pay_form.jsp'">
            </td>
        </tr>
    </table>
</div>
